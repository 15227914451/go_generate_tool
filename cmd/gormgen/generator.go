...
func (g *Generator) SetImportPkg(importPkgs []ImportPkg) *Generator {
	fmt.Println("importPkgs : ", importPkgs)
...
// ParserAST parse by go file modefiy by dzd
func (g *Generator) ParserASTPath(p *Parser, structs []string, inputPath string) (ret *Generator) {
	for _, v := range structs {
		fmt.Println("key is gorm.ToDBName(v):", gorm.ToDBName(v))
		g.buf[gorm.ToDBName(v)] = new(bytes.Buffer)

	}
	g.structConfigs = p.Parse()
	fmt.Println(p.pkg.Name)
	inputPathRep := strings.ReplaceAll(inputPath, "../", "")
	g.config.PkgName = inputPathRep
	return g
}
...
//Generate controller
func (g *Generator) GenerateController() *Generator {
	if err := g.checkConfig(); err != nil {
		panic(err)
	}
	...
	for _, v := range g.structConfigs {
		...
		if err := outputControllerTemplate.Execute(g.buf[gorm.ToDBName(v.StructName)], v); err != nil {
			panic(err)
		}
	}
...
func (g *Generator) Flush() error {
	for k, _ := range g.buf {
		filename := g.inputFile + "/" + strings.ToLower(k) + "Service.go"
...
// Flush controller function writes the output to the output file.
func (g *Generator) FlushController() error {
	for k, _ := range g.buf {
		filename := g.inputFile + "/" + strings.ToLower(k) + "Controller.go"
		if err := ioutil.WriteFile(filename, g.buf[k].Bytes(), 0777); err != nil {
			log.Fatalln(err)
		}
	}
